class Paciente:
    _id_counter = 1

    def __init__(self, nome, data_nascimento, telefone, email):
        self.id = Paciente._generate_id()
        self.nome = nome
        self.data_nascimento = data_nascimento
        self.telefone = telefone
        self.email = email
        self.consultas = []
        self.cirurgias = []
        self.medicamentos_prescritos = []

    @classmethod
    def _generate_id(cls):
        current_id = cls._id_counter
        cls._id_counter += 1
        return current_id

    def __str__(self):
        return f"Paciente(ID: {self.id}, Nome: {self.nome}, Data de Nascimento: {self.data_nascimento}, Telefone: {self.telefone}, Email: {self.email})"

    def adicionar_consulta(self, consulta):
        self.consultas.append(consulta)
        print(f"{self.nome} Consulta adicionada: {consulta.descricao}")

    def remover_consulta(self, consulta):
        if consulta in self.consultas:
            self.consultas.remove(consulta)
            print(f"Consulta removida: {consulta.descricao}")

    def atualizar_consulta(self, consulta, nova_descricao=None, nova_data=None, novo_medico=None, novo_departamento=None):
        if nova_descricao:
            consulta.descricao = nova_descricao
        if nova_data:
            consulta.data = nova_data
        if novo_medico:
            consulta.medico = novo_medico
        if novo_departamento:
            consulta.departamento = novo_departamento
        print(f"Consulta atualizada: {consulta}")

    def visualizar_consultas(self):
        return [str(consulta) for consulta in self.consultas]

    def adicionar_cirurgia(self, cirurgia):
        self.cirurgias.append(cirurgia)
        print(f"Cirurgia adicionada: {cirurgia.descricao}")

    def remover_cirurgia(self, cirurgia):
        if cirurgia in self.cirurgias:
            self.cirurgias.remove(cirurgia)
            print(f"Cirurgia removida: {cirurgia.descricao}")

    def atualizar_cirurgia(self, cirurgia, nova_descricao=None, nova_data=None, novo_medico=None, novo_departamento=None):
        if nova_descricao:
            cirurgia.descricao = nova_descricao
        if nova_data:
            cirurgia.data = nova_data
        if novo_medico:
            cirurgia.medico = novo_medico
        if novo_departamento:
            cirurgia.departamento = novo_departamento
        print(f"Cirurgia atualizada: {cirurgia}")

    def visualizar_cirurgias(self):
        return [str(cirurgia) for cirurgia in self.cirurgias]

    def historico_medico(self):
        return {
            "consultas": self.visualizar_consultas(),
            "cirurgias": self.visualizar_cirurgias()
        }

    def visualizar_observacoes_cirurgia(self, cirurgia):
        if cirurgia in self.cirurgias:
            return cirurgia.visualizar_observacoes()
        else:
            print("Cirurgia não encontrada.")
            return None

    def visualizar_resultado_cirurgia(self, cirurgia):
        if cirurgia in self.cirurgias:
            return cirurgia.visualizar_resultado()
        else:
            print("Cirurgia não encontrada.")
            return None

    def adicionar_medicamento_prescrito(self, medicamento, observacao):
        self.medicamentos_prescritos.append({'medicamento': medicamento, 'observacao': observacao})

    def visualizar_medicamentos_prescritos(self):
        if not self.medicamentos_prescritos:
            print("Nenhum medicamento prescrito.")
        else:
            return [f"{med['medicamento']} - Observação: {med['observacao']}" for med in self.medicamentos_prescritos]
    